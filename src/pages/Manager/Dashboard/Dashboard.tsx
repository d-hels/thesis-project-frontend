...
//   ];
  ...
  // Card stats
  const stats = [
    { title: "Total Employees", value: numberOfEmployees, color: "#40c9c6", icon: <TeamOutlined /> },
    { title: "Active Projects", value: 15, color: "#36a2eb", icon: <FileTextOutlined /> },
    { title: "Pending Requests", value: 5, color: "#f6a623", icon: <BellOutlined /> },
    { title: "Completed Tasks", value: 230, color: "#ff6b6b", icon: <DashboardOutlined /> },
  ];
    ...
    <Layout style={{ minHeight: "100vh" }}>
      <Layout>
        <Content style={{ margin: "24px", overflow: "initial" }}>
          <Row gutter={[24, 24]} style={{ marginBottom: 24 }}>
            {stats.map((stat) => (
              <Col xs={24} sm={12} md={6} key={stat.title}>
                ...
                >
                  <div>
                    <Text style={{ color: "#fff", fontWeight: 500 }}>{stat.title}</Text>
                    <Title level={2} style={{ color: "#fff", margin: 0 }}>{stat.value}</Title>
                  </div>
          ...
          <Card title="Employees Overview" bordered={false} style={{ borderRadius: 12, boxShadow: "0 4px 12px rgba(0,0,0,0.05)" }}>
